...
def main():
	...
	parser.add_argument("--data_dir",
						default='TAS/data/absa_dataset/three_joint/TO/',
						type=str,
						required=True,
	...
	if os.path.exists(args.output_dir) and os.listdir(args.output_dir): # If the directory exists, remove it and its contents
	...
	if args.use_crf:
		model = BertForTABSAJoint_CRF(args.model_name , len(label_list), len(ner_label_list))
	else:
		model = BertForTABSAJoint(args.model_name, len(label_list), len(ner_label_list), args.max_seq_length)
	...
	with open(output_log_file, "w") as writer:
		if args.eval_test:
			writer.write("epoch\tglobal_step\tloss\ttest_loss\ner_test_loss\ttest_accuracy\n")
		else:
			writer.write("epoch\tglobal_step\tloss\tner_loss\n")
	...
	for _ in trange(int(args.num_train_epochs), desc="Epoch"):
		...
		if args.eval_test:
			...
			with open(os.path.join(args.output_dir, "test_ep_"+str(epoch)+".txt"),"w") as f_test:
				...
				for input_ids, input_mask, segment_ids, label_ids, ner_label_ids, ner_mask in test_dataloader:
					...
					for output_i in range(len(outputs)):
						...
						#label_true = ner_label_ids[output_i].flatten()
						#label_pre = ner_logits[output_i].flatten()
			...
			#micro_f1_score = f1_score(label_ids, all_label_pre, average='micro')  # Convert to percent
