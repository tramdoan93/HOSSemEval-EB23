...
def evaluate_SD(path, best_epoch_file):
    with open(os.path.join(path, TXT_file(best_epoch_file)), 'r', encoding='utf-8') as f_pre:
        ...
        for line in pre_lines:
            ...
            sentiment_true = pre_line[-1].split()[1] if gold_label == 1 else 'None'
            sentiment_pred = pre_line[-1].split()[1] if predicted_label == 1 else 'None'
...
def evaluate_AD(path, best_epoch_file):
    with open(os.path.join(path, TXT_file(best_epoch_file)), 'r', encoding='utf-8') as f_pre:
        Gold_Num = 0  # Actual positives
        True_Num = 0  # True positives (correct aspect prediction)
        Pre_Num = 0   # Predicted positives
        ...
        for line in pre_lines:
            ...
            aspect_pred = pre_line[-1].split()[0] if predicted_label == 1 else 'None'
        ...
        # Print classification report
        print('Classification Report:')
        print(classification_report(y_true, y_pred, labels=['Facility', 'Amenity', 'Service', 'Experience', 'Branding', 'Loyalty' ]))
...
if __name__ == '__main__':
	...
	parser.add_argument("--best_epoch_file_ad_sd",
						type=str,
						required=True,
						default=30,
						help="The epochs num in training & testing")
	...
	evaluate_AD(args.output_dir, best_epoch_file_ad_sd)
	evaluate_SD(args.output_dir, best_epoch_file_ad_sd)	
